...
###### Inputs
The example folder contains the sample inputs **example_input.gff**, **example_input.out** (GFF and OUT files from RepeatMasker), **example_ltrfinder.gff**. Currently RepeatCraft accept the GFF generated by RepeatMasker v4.0.7 and v4.0.6 only. The table output from LTR_FINDER can be converted to GFF by using `cnv_ltrfinder2gff.pl` from the [DAWGPAWS pipeline](https://github.com/jestill/dawgpaws).
The configuration file **repeatcraft.cfg** and the mapfile **mapfile.tsv** which is used in short TEs labelling. 
